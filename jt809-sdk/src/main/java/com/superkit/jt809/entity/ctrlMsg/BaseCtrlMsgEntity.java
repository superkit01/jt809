...
/**
 * 主链路车辆监管业务 链路类型:主链路 消息方向:下级平台往上级平台 业务数据类型标识:UP_CTRL_MSG 描述:下级平台向上级平台发送车辆监管业务
 * <p>
 * <p>
 * 从链路车辆监管业务 链路类型:从链路 消息方向:上级平台往下级平台 业务数据类型标识:DOWN_CTRL_MSG 描述:上级平台向下级平台发送报瞥信息业务
 */
@Setter
@Getter
@NoArgsConstructor
public class BaseCtrlMsgEntity implements Codec<BaseCtrlMsgEntity>{
	...
	@Override
	public BaseCtrlMsgEntity decode(ByteBuf buf) {
	...
	public static BaseCtrlMsgEntity create(String vehicleNo, byte vehicleColor, int dataType, byte[] data) {
		BaseCtrlMsgEntity baseMsg = new BaseCtrlMsgEntity();
		...
		baseMsg.base = BaseEntity.create(MsgConstant.PrimaryServiceType.UP_CTRL_MSG, msgbody);
		return baseMsg;
	}
