...
public class NettyServerHandler extends ChannelInboundHandlerAdapter {
	...
	public NettyServerHandler(ProcessorManager processorManager) {
	...
	public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
		log.warn("server-exceptionCaught");
		log.error("NettyServer-exceptionCaught", cause);
	}
	...
	public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
		log.warn("server-userEventTriggered");
	}

	@Override
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		log.warn("server-channelInactive");
	}
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws InterruptedException {
		try {
			...
			log.debug("接收报文:{}", BufUtil.buf2HexStr(byteBuf));
			 ...
			 * 链路管理
			 */
			case MsgConstant.PrimaryServiceType.DOWN_CONNECT_REQ: {
				log.info("【接收】上级平台从链路登录请求");
				processorManager.getLinkManagerProcessor().receiveSubConnectReq(byteBuf, ctx.channel());
				break;
			}
			case MsgConstant.PrimaryServiceType.DOWN_DISCONNECT_REQ: {
				log.info("【接收】从链路注销请求");
				processorManager.getLinkManagerProcessor().receiveSubDisConnectReq(byteBuf, ctx.channel());
				break;
			}
			case MsgConstant.PrimaryServiceType.DOWN_LINKTEST_REQ: {
				log.info("【接收】从链路心跳");
				processorManager.getLinkManagerProcessor().receiveSubLinkTestReq(byteBuf, ctx.channel());
				break;
			}
			/**
			 * 信息统计
